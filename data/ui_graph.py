...
class Interaction(Data,Graph):
    def __init__(self, conf, training, test,mode=''):
        Graph.__init__(self)
        Data.__init__(self,conf,training,test)
        self.mode = mode
        self.dataset_dir = f"./dataset/{conf['dataset']}"
        self.user = {}
        self.item = {}
        self.id2user = {}
        self.id2item = {}
    ...
    def _append_training_data(self,rdm_itm_,i):
        self.training_data.append(([rdm_itm_, i, float(1)]))
    ...
    def __create_sparse_bipartite_adjacency(self, self_connection=False):
        ...
        col_idx_raw = [pair[1] for pair in self.training_data]
        if self.mode =='train':
            import pandas as pd
            self.train_value_count = pd.Series(col_idx_raw).value_counts()
            popular_item_list = pickle.load(open("{}/popular_item_list.pkl".format(self.dataset_dir), "rb"))
            popular_item_list_int = [self.item.get(_,-1) for _ in popular_item_list]
            popular_item_list_int = [_ for _ in popular_item_list_int if _ >=0]
            pickle.dump(popular_item_list_int,open("{}/popular_item_list_int.pkl".format(self.dataset_dir), "wb"))
            _head_index = np.in1d(np.array(col_idx_raw),np.array(popular_item_list))
            self.head_index = item_np[_head_index]
            self.tail_index = item_np[~_head_index]

        tmp_adj = sp.csr_matrix((ratings, (user_np, item_np + self.user_num)), shape=(n_nodes, n_nodes),dtype=np.float32)
        adj_mat = tmp_adj + tmp_adj.T
        if self_connection:
            adj_mat += sp.eye(n_nodes)
        print('finish the matrix')
        return adj_mat
    ...
    def convert_to_laplacian_mat_galr(self, tmp_adj):
        tmp_adj = tmp_adj + tmp_adj.T
        return self.normalize_graph_mat(tmp_adj)
